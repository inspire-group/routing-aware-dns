...
def lookupName(name, record, recursion_limit=10, resolve_all_glueless=True, 
               master_timeout=10):
    #return lookupNameRecursive(name, record, 8, False, 3)
    return lookupNameRecursive(name, record, recursion_limit, resolve_all_glueless, master_timeout)
  ...
  while True:
    ...
    while True:
      ...
      if isinstance(ipOrLookup, str):
        ...
        if (nameserverName, dns.rdatatype.A) in cache and cache[(nameserverName, dns.rdatatype.A)] is None:
    ...
    if response.answer != []:
      ...
      if resolveAllGlueless:
        for gluelessNameServer in listOfGluelessNameServers:
          if (gluelessNameServer, dns.rdatatype.A) in cache and cache[(gluelessNameServer, dns.rdatatype.A)] is None:
...
# This function is deprecated. AAAA records need to be looked up in another lookup to spport IPv6.
...
def performFullLookupForName(name):
    ...
    try:
        ...
        if "NoAnswer" in str(lookupError):
            ...
            try:
                lookupv4 = lookupName(name, dns.rdatatype.A, resolve_all_glueless=False)
                aRecords = getAllAddressesForHostnameFromResultChain(lookupv4)
                (lookup4DNSIPsv4, lookup4DNSIPsv6) = getFullDNSTargetIPList(lookupv4)
                matchedBackupResolverv4 = checkMatchedBackupResolver(lookupv4)
    ...
    try:
        ...
        # to attempt to get a valid lookup, we can redo the lookup with resolve all gleuless as false.
            try:
                lookupv6 = lookupName(name, dns.rdatatype.AAAA, resolve_all_glueless=False)
                aaaaRecords = getAllAddressesForHostnameFromResultChain(lookupv6)
                (lookup6DNSIPsv4, lookup6DNSIPsv6) = getFullDNSTargetIPList(lookupv6)
                matchedBackupResolverv6 = checkMatchedBackupResolver(lookupv6)
            except ValueError as lookupError2:
